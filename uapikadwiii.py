...
def animasi_struk():
    ...
    for i in range(5):
        cprint(("-" * (i + 1), "white", "on_green"), end='\r')
        time.sleep(0.2)
    cprint(("- Selesai!", "green", "on_yellow"))
...
def simpan_data(nama, keranjang):
...
def cetak_struk(keranjang, total, bayar, kembali):
    cprint("===== STRUK PEMBELIAN =====", "white", "on_blue")
    for item in keranjang:
        nama_obat = item[0]
        jumlah = item[1]
        harga_satuan = item[2]
